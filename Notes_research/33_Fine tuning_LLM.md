...
| Type | Description | Example Prompts | Data Needs | Compute |
|------|-------------|-----------------|------------|---------|
| **Instruction** | Task + instruction → free-form response | "Is this spam? Answer yes/no"<br>"Translate to German:" | Large, diverse | High |
| **Classification** | Input → **fixed category** (no instruction) | Email → "spam"/"ham" | Smaller, balanced | Lower |

**Today's focus**: **Classification fine-tuning** (simpler, educational).

**Advanced (future lectures)**:
- **LoRA**: Train **tiny adapters** (~0.1% params) → memory-efficient.
- **QLoRA**: **Quantized LoRA** → even lower memory.

---

## 3. Hands-on: Spam Classification Dataset

**Dataset**: [SMS Spam Collection](https://archive.ics.uci.edu/dataset/228/sms+spam+collection)
```

Total: 5,572 SMS messages
...
spam   "Free entry in 2 win £100..."
```


### Step 2: Balance Dataset (Simple Approach)

**Problem**: Imbalanced → model learns "always ham".
**Solution**: Subsample ham to match spam count (747 each).

```python
def create_balanced_df(df):
    spam_df = df[df.label == "spam"]      # 747 spam
    ham_df = df[df.label == "ham"].sample(n=747, random_state=42)  # Sample 747 ham
...
balanced_df.to_csv("balanced_sms.csv", index=False)
```
...
def random_split(df, train_frac=0.7, val_frac=0.1):
    ...
    val_df = remaining_df.sample(frac=val_frac/(1-train_frac), random_state=42)
    test_df = remaining_df.drop(val_df.index)
    return train_df, val_df, test_df

train_df, val_df, test_df = random_split(balanced_df)
print(f"Train: {len(train_df)}, Val: {len(val_df)}, Test: {len(test_df)}")
# Train: 1045, Val: 149, Test: 300 ✓ (1045+149+300=1494)
```
...
## 4. Fine-tuning Pipeline (5-6 Lectures)

```
1. **Today**: Download + Preprocess (✅)
2. **Next**: DataLoaders + Model Setup + Pretrained Weights
3.       Augment GPT → Add classification head
4.       Training loop (cross-entropy on spam/ham)
5.       Evaluation (accuracy, F1 on val/test)
6.       Inference on new SMS
```

**Classification head** (coming soon):

```
GPT-2 logits [batch, seq, 50257] → pool → Linear(768 → 2) → softmax → [spam/ham]
```
...
```
Raw:      5,572 emails (86% ham, 13% spam)
Balanced: 1,494 emails (50/50)
Train:    1,045 (70%)
Val:        149 (10%)
Test:      300 (20%)
```

**Why balance?** Educational → equal classes, small dataset.

**Production**: Use **weighted loss** or **oversampling**.

### Label Mapping (Like Token IDs)

```
ham → 0    (like token ID 0)
spam → 1   (like token ID 1)
```

**Simpler** than GPT vocab (50K tokens → 2 classes).
...
```python
# Sanity checks
assert len(train_df) + len(val_df) + len(test_df) == len(balanced_df)
assert balanced_df.label.value_counts().min() == 747
print("✅ Dataset ready!")
```
...
```
1. DataLoaders (batch emails → tokens)
2. Load GPT-2 pretrained weights
3. **Augment model**: GPT + Classification Head
   GPT embeddings → Linear(768→2) → [spam/ham probs]
4. Loss: CrossEntropy (target: 0/1 labels)
5. Training loop + Evaluation
```

**Goal**: **95%+ accuracy** on spam detection!

```
Philosophy: Detailed whiteboard + code → No rushing
Next: DataLoaders + Model modification
```
