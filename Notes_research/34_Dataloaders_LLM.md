...
class SpamDataset(torch.utils.data.Dataset):
    def __init__(self, csv_file, tokenizer, max_length=None, pad_token_id=50256):
        ...
        # Find longest sequence (if not specified)
        if max_length is None:
            self.max_length = self._longest_encoded_length()
        else:
            self.max_length = max_length
            
        # Tokenize + pad ALL sequences
        self.encoded = []
        for text in self.data['message']:
        ...
        self.labels = self.data['label'].values  # 0=ham, 1=spam
    
    def _longest_encoded_length(self):
        return max(len(self.tokenizer.encode(text)) 
                  for text in self.data['message'])
...
# Training set → auto-detect max_length
train_dataset = SpamDataset("train.csv", tokenizer)
print(f"Max length: {train_dataset.max_length}")  # ~120 tokens

# Val/Test → use train's max_length (truncate if needed)
...
- `shuffle=True` (train only): Randomize order
- `drop_last=True`: Drop incomplete final batch
- `num_workers=0`: No parallel loading (simplicity)

---

## 6. Batch Verification

### Step 6.1: Inspect First Batch
...
print(f"Input batch:  {xb.shape}")   #[^1]
print(f"Label batch: {yb.shape}")    #[^1]
print(f"Sample tokens: {xb[:20]}") # Token IDs
print(f"Sample label:  {yb}")     # 0 or 1
```

**Expected**:

```
Input batch:  torch.Size()[^1]
Label batch:  torch.Size()[^1]
```


### Step 6.2: Full Dataset Stats

```python
print(f"Train batches: {len(train_loader)}")  # ~130
print(f"Val batches:   {len(val_loader)}")    # ~19  
...
```
Train samples: 1,045 ÷ 8 = 130 batches ✓
Val samples:   149 ÷ 8 = 19 batches  ✓
Test samples:  300 ÷ 8 = 38 batches  ✓
```


---

## 7. Visual Batch Structure

```
Input batch:[^1]
Row 0: [5037, 878,  ..., 50256, 50256...]  # Email 1 tokens
Row 1: [3187, 265,  ..., 50256, 50256...]  # Email 2 tokens  
...
Row 7: [..., ..., ..., 50256, ...]         # Email 8 tokens

Label batch:[^1]
  # ham=0, spam=1[^2]
```

**Perfect for model**: Fixed `[batch_size, seq_len]` + labels `[batch_size]`.

---

## 8. Key Differences: Pretraining vs Fine-tuning

| Aspect | Pretraining | **Fine-tuning (Classification)** |
| :-- | :-- | :-- |
| **Input** | Token sequence `[B, T]` | Token sequence `[B, T]` |
| **Target** | **Next tokens** `[B, T]` (shifted) | **Class labels** `[B]` (0/1) |
| **Loss** | CrossEntropy(vocab=50K) | CrossEntropy(classes=2) |
| **Output** | Logits `[B, T, 50K]` | Logits `[B, 2]` |

**Model modification needed**: GPT → **Classification Head**.
...
├── Balance ✓  
├── Split ✓
└── DataLoaders ✓ [8×120 tokens, 8 labels]

Stage 2: Model Setup (Next)
├── Load GPT-2 weights
├── Add classification head: Linear(768→2)
└── Training loop
```

**Ready for training**: `train_loader` → `model(xb) → loss(logits, yb)`.
...
```
✅ Dataset class: Tokenize + pad to max_length=120
✅ DataLoaders: 130 train / 19 val / 38 test batches  
✅ Batch shape:  tokens +  labels ✓[^1]
✅ Padding: <|endoftext|> (50256) ✓
...
```
train_loader → [130 batches × (8 emails, 8 labels)]
val_loader  → [19 batches × (8 emails, 8 labels)]
test_loader → [38 batches × (8 emails, 8 labels)]
```

```
Next lecture: Load GPT-2 + Classification Head + Training!
```


---

**Victory**: **Production-ready batches** for spam classification! 🎉

<div align="center">⁂</div>

[^1]: https://dictionary.cambridge.org/dictionary/english-chinese-traditional/contemplate

[^2]: 28-And-29-Spam-Ham-Projects-Using-Word2vec-AvgWord2vec.ipynb
