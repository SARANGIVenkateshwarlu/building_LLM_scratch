...
**Core idea:**
LLMs learn $P(\text{next token} \mid \text{previous tokens})$
...
Convert raw text into model-ready numerical inputs.
...
- Add Positional Embeddings → Input Embeddings
...
- Token embedding = meaning of word
- Positional embedding = where word appears
...
- Tokenization impacts performance heavily (BPE, WordPiece).
...
## 3. Attention Mechanism (Core Innovation)

### What

Transforms input embeddings → **context-aware vectors**
...
6. Multiply with V → context vectors
...
Like multiple “perspectives” reading the same sentence.

### Key Insight (Added)

- Attention replaces recurrence → enables parallelization.
- Complexity: $O(n^2)$ → bottleneck for long sequences.
...
- Input embeddings
- → Transformer blocks (e.g., 12 in GPT-2)
- → Final linear layer
- → Logits (vocab probabilities)
...
- Feedforward expands dimension → adds non-linearity.
...
$$
\text{Loss} = \text{CrossEntropy}(y_{true}, y_{pred})
$$
...
Model learns by minimizing surprise.

***

## 6. Training (Pre-training)

### What

Optimize millions/billions of parameters.
...
Learn language patterns from large corpora.

### Intuition

Gradually adjusts model to reduce prediction error.

### Key Insight (Added)

- Training data scale = key to intelligence.
- Compute cost: millions of dollars for large models.
- Small-scale training still teaches mechanics.
...
Load weights from models like GPT-2.
...
Reuse learned language knowledge.
...
Pre-training ≠ task-specific performance.
...
Train LLM for classification (e.g., spam detection)

#### Intuition

Use LLM as feature extractor + classifier.
...
- Input: "Convert to passive"
- Output: transformed sentence


#### Why

Makes model usable like ChatGPT.

#### Intuition

Teach model how to follow directions.
...
- This is foundation of:
    - Chatbots
    - Assistants
- Often combined with RLHF (not covered in lecture).
...
- 57 tasks across domains


#### 2. Human Evaluation

- Subjective but high-quality


#### 3. LLM-based Evaluation

- Use strong model (e.g., LLaMA 3) to score outputs


### Intuition

Evaluation = measuring usefulness, not just accuracy.

### Key Insight (Added)

- Automatic metrics often fail for open-ended tasks.
- LLM-as-a-judge is becoming standard.
...
4. Add positional info
5. Pass through Transformer
6. Generate logits
7. Predict next token
8. Compute loss
9. Backpropagation
10. Update weights
...
## 12. Practical Insights for Interviews

- LLM = **autoregressive probabilistic model**
- Attention = **context modeling**
- Transformer = **stack of attention + MLP blocks**
- Pre-training = **language learning**
...
- Why scaling factor $\sqrt{d_k}$?
- Why multi-head attention?
- Difference: embedding vs positional embedding?
...
## 14. Research Directions (Added)

- Smaller efficient models (Tiny LLMs)
- Long-context attention (linear attention)
...
- Transformer → deep reasoning engine
...
- **flashcards for interviews**
- or **code + math deep dive version**
